...
AddEventHandler('t1ger_deliveryjob:fetchCompanyCL', function(companyID)
	...
	HHCore.TriggerServerCallback('t1ger_deliveryjob:getTakenCompanies', function(ownedCompanies)
...
function BuyCompanyMenu(id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "company_purchase_menu",
		{
	...
	function(data, menu)
		if(data.current.value == 'confirm_purchase') then
			...
			HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'company_choose_name_dialog', {
				title = "Enter Name For Company"
			}, function(data2, menu2)
				menu2.close()
				local firmName = tostring(data2.value)
				HHCore.TriggerServerCallback('t1ger_deliveryjob:buyCompany', function(purchased)
					if purchased then
						ShowNotifyHHCore((Lang['company_bought']):format(math.floor(val.price)))
...
function ManageCompanyMenu(id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "company_manage_menu",
		{
	...
	function(data, menu)
		if(data.current.value == 'rename_company') then
			HHCore.UI.Menu.Open('dialog', GetCurrentResourceName(), 'company_rename_dialog', {
				title = "Rename Company"
			}, function(data2, menu2)
				menu2.close()
				local firmName = tostring(data2.value)
				HHCore.TriggerServerCallback('t1ger_deliveryjob:renameCompany', function(renamed)
					if renamed then
						ShowNotifyHHCore((Lang['company_renamed']):format(firmName))
						TriggerServerEvent('t1ger_deliveryjob:fetchCompanySV')
						menu.close()
						deliveryMenu = nil
					else
						ShowNotifyHHCore(Lang['not_your_company'])
						menu.close()
						deliveryMenu = nil
					end
				end, id, val, firmName)
...
function SellCompanyMenu(id,val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "company_sell_menu",
		{
	...
	function(data, menu)
		if(data.current.value == 'confirm_sale') then
			HHCore.TriggerServerCallback('t1ger_deliveryjob:sellCompany', function(sold)
				if sold then
					TriggerServerEvent('t1ger_deliveryjob:fetchCompanySV')
					ShowNotifyHHCore((Lang['company_sold']):format(math.floor(sellPrice)))
				else
					ShowNotifyHHCore(Lang['not_your_company'])
				end
...
function CompanyLevelMenu(id,val)
	...
	HHCore.TriggerServerCallback('t1ger_deliveryjob:getCompanyData', function(level, certifcate)
		if level ~= nil then
			...
			HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "company_skill_level",
				{
			...
			function(data, menu)
				...
				if(data.current.value == 'buy_certifcate') then
					HHCore.TriggerServerCallback('t1ger_deliveryjob:buyCertifcate', function(certificateStatus)
						if certificateStatus == nil then 
							ShowNotifyHHCore(Lang['not_your_company'])
						elseif certificateStatus == 1 then 
							ShowNotifyHHCore(Lang['alrdy_has_certificate'])
						elseif certificateStatus == true then 
							ShowNotifyHHCore(Lang['certificate_acquired'])
...
function RequestJobMenu(id,val)
	...
	HHCore.TriggerServerCallback('t1ger_deliveryjob:getCompanyData', function(level, certifcate)
		if level ~= nil then
			...
			HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "request_job_menu",
				{
			...
			function(data, menu)
				if level >= data.current.level then 
					...
					elseif data.current.certificate == true and certifcate == 1 then 
						...
						ShowNotifyHHCore(Lang['job_needs_certificate'])
					end
				else
					ShowNotifyHHCore(Lang['job_level_mismatch'])
				end
			end, function(data, menu)
				menu.close()
				ManageCompanyMenu(id,val)
...
function ShopOrderMenu(id, val)
	HHCore.TriggerServerCallback('t1ger_deliveryjob:fetchShopOrders', function(orders)
		local elements = {}
		if #orders > 0 then
			...
			HHCore.UI.Menu.CloseAll()
			HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "shop_order_list",
				{
			...
			ShowNotifyHHCore(Lang['no_available_orders'])
		end
	end)
end
...
function SelectJobVehicle(jobValue, label, level, certificate, vehicles, orderData, id, val)
	...
	HHCore.UI.Menu.Open('default', GetCurrentResourceName(), "job_select_vehicle",
		{
	...
	function(data, menu)
		HHCore.TriggerServerCallback('t1ger_deliveryjob:vehDepositFee', function(deposited)
			if deposited then
				vehDeposit = data.current.deposit
				ShowNotifyHHCore((Lang['deposit_veh_paid']:format(data.current.deposit)))
				menu.close()
				...
				ShowNotifyHHCore(Lang['not_enough_to_deposit'])
			end
...
AddEventHandler('t1ger_deliveryjob:highValueDelivery', function(jobValue, label, level, certificate, vehModel, id, val)
	...
	while true do
		...
		if not forkliftSpawned then
			...
			HHCore.Game.SpawnVehicle(val.forklift.model, {x = jk.pos[1], y = jk.pos[2], z = jk.pos[3]}, jk.pos[4], function(veh)
					...
					if IsControlJustPressed(0, Config.KeyToLoadJobVehicle) then
						...
						if curVeh > 0 then 
							if GetEntityModel(curVeh) == GetEntityModel(jobForklift) then
								...
								ShowNotifyHHCore(Lang['forklift_mismatch'])
							end
						else
							ShowNotifyHHCore(Lang['not_inside_forklift'])
						end
					end
				end
			end
		...
		if traillerFilledUp and not truckingStarted then 
			...
			if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunk.x, trunk.y, trunk.z, true) < 5.0) then 
				...
				if IsControlJustPressed(0, 47) then
					...
					DoScreenFadeIn(1000)
					Citizen.Wait(100)
					ShowNotifyHHCore(Lang['trailer_filled_up'])
					SetTruckingRoute()
				end
			end
		end
						...
						if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunk.x, trunk.y, trunk.z, true) < 5.0) and jobForklift == nil then 
							...
							if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunk.x, trunk.y, trunk.z, true) < 3.5) then 
								if IsControlJustPressed(0, 47) then
									...
									HHCore.Game.SpawnVehicle(val.forklift.model, {x = trunk.x, y = trunk.y, z = trunk.z}, GetEntityHeading(GetPlayerPed(-1)), function(veh)
									...
									HHCore.Game.SpawnObject(Config.HighValueJobs[jobNum].prop, {x = pSpot[1], y = pSpot[2], z = pSpot[3]}, function(pallet)
						...
						if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, dSpot[1], dSpot[2], dSpot[3], true) < 4.0) then
							...
							if IsControlJustPressed(0, 38) then
								if not IsEntityInAir(palletObjEntity) then
									...
									ShowNotifyHHCore(Lang['park_fork_in_trailer'])
								else
									ShowNotifyHHCore(Lang['place_pallet_on_ground'])
								end
							end
						end
					end
						...
						if IsControlJustPressed(0, Config.KeyToReturnJobVehicle) then
							...
							if curVeh > 0 then
								if GetEntityModel(curVeh) == GetEntityModel(jobVehicle) then
									...
									ShowNotifyHHCore(Lang['job_veh_mismatch'])
								end
							else
								ShowNotifyHHCore(Lang['sit_in_job_veh'])
							end
						end
					end
				end
...
function PalletDeliveryPay()
	...
	if newVehBody < (truckHealth*dmgPercent) then 
		ShowNotifyHHCore(Lang['pallet_damaged_transit'])
		paycheck = paycheck
	else
		paycheck = paycheck + palletPrice
		ShowNotifyHHCore((Lang['paycheck_add_amount']:format(palletPrice)))
	end
end
...
function forkliftIntoTruck(palletSpots, objMarker, prop, jobValue)
	...
	for num,v in pairs(palletSpots) do
		HHCore.Game.SpawnObject(prop, {x = v[1], y = v[2], z = v[3]}, function(pallet)
            SetEntityHeading(pallet, 150.0)
            SetEntityAsMissionEntity(pallet, true, true)
            PlaceObjectOnGroundProperly(pallet)
            Wait(1000)
								...
								if deliveredParcels < maxDeliveries then
									if IsEntityAttachedToAnyPed(deliveryParcel) then 
										...
										elseif deliveredParcels == maxDeliveries then 
											...
											ShowNotifyHHCore(Lang['delivery_complete'])
											if jobValue == 4 then
										...
										ShowNotifyHHCore(Lang['parcel_not_ind_hand'])
									end
								end
							end
						end
...
function ReturnVehAndGetPaycheck()
	...
	if newVehBody < (1000*dmgDeposit) then 
		giveDeposit = false
		ShowNotifyHHCore(Lang['deposit_not_returned'])
	else
		giveDeposit = true
	end
...
function ParcelDeliveryPay()
	...
	if newVehBody < (vehBodyHealth*dmgPercent) then 
		ShowNotifyHHCore(Lang['parcel_damaged_transit'])
		paycheck = paycheck
	else
		paycheck = paycheck + parcelPrice
		ShowNotifyHHCore((Lang['paycheck_add_amount']:format(parcelPrice)))
	end
end
			...
			if (GetDistanceBetweenCoords(coords.x, coords.y, coords.z, trunk.x, trunk.y, trunk.z, true) < 2.0) then 
				...
				if IsControlJustPressed(0, Config.KeyToPutParcelInVeh) then
					...
					if totalObjects == 0 then 
						...
						ShowNotifyHHCore(Lang['vehicle_filled_up'])
						fillingVeh = false
					end
				end
			end
...
function SpawnJobVehicle(vehModel, vehCoords, vehHeading)
	HHCore.Game.SpawnVehicle(vehModel, {x = vehCoords[1], y = vehCoords[2], z = vehCoords[3]}, vehHeading, function(veh)
	...
	ShowNotifyHHCore(Lang['job_veh_spawned'])
end

function SpawnTruckTrailer(trailerModel, vehCoords, vehHeading)
	HHCore.Game.SpawnVehicle(trailerModel, {x = vehCoords[1], y = vehCoords[2], z = vehCoords[3]}, vehHeading, function(trailer)
