...
-- Get Owned Companies:
HHCore.RegisterServerCallback('t1ger_deliveryjob:getTakenCompanies',function(source, cb)
...
-- Purchase Company:
HHCore.RegisterServerCallback('t1ger_deliveryjob:buyCompany',function(source, cb, id, val, name)
    local xPlayer = HHCore.GetPlayerFromId(source)
    local money = 0
...
-- Sell Company:
HHCore.RegisterServerCallback('t1ger_deliveryjob:sellCompany',function(source, cb, id, val, sellPrice)
    local xPlayer = HHCore.GetPlayerFromId(source)
...
-- Reanme Company:
HHCore.RegisterServerCallback('t1ger_deliveryjob:renameCompany',function(source, cb, id, val, name)
    local xPlayer = HHCore.GetPlayerFromId(source)
    MySQL.Async.fetchAll("SELECT companyID FROM t1ger_deliveryjob WHERE identifier = @identifier", {['@identifier'] = xPlayer.identifier}, function(data)
        if data[1].companyID ~= nil then 
            if data[1].companyID == id then
...
HHCore.RegisterServerCallback('t1ger_deliveryjob:buyCertifcate',function(source, cb, id)
    local xPlayer = HHCore.GetPlayerFromId(source)
...
HHCore.RegisterServerCallback('t1ger_deliveryjob:vehDepositFee',function(source, cb, amount)
    local xPlayer = HHCore.GetPlayerFromId(source)
    local plyMoney = 0
...
AddEventHandler('t1ger_deliveryjob:retrievePaycheck', function(paycheck, vehDeposit, giveDeposit)
    ...
    if giveDeposit then
        ...
        TriggerClientEvent('t1ger_deliveryjob:ShowNotifyHHCore', xPlayer.source, (Lang['deposit_returned']:format(vehDeposit)))
    end
    xPlayer.addMoney(paycheck)
    TriggerClientEvent('t1ger_deliveryjob:ShowNotifyHHCore', xPlayer.source, (Lang['paycheck_received']:format(paycheck)))
...
HHCore.RegisterServerCallback('t1ger_deliveryjob:fetchShopOrders',function(source, cb)
    local xPlayer = HHCore.GetPlayerFromId(source)
